...
class Config_Parser(object):
    ...
    def read_server(filename='app1.cnf'):
        ...
        if sections_count == 1:
            print('你的cnf配置文件只设置了一个MariaDB，至少两个MariaDB做一个主从复制，才满足最基本的高可用故障切换架构！')
            sys.exit('MariaDB Replication Health is NOT OK!')
                ...
                if r_dict['Slave_IO_Running'] == 'Yes' and r_dict['Slave_SQL_Running'] == 'Yes':
                    ...
                    logging.error('\033[1;31m主从复制报错，请检查. Slave_IO_Running状态值是：%s '
                                  ...
                                  % (r_dict['Slave_IO_Running'], r_dict['Slave_SQL_Running'], \
                                     r_dict['Last_IO_Error'], r_dict['Last_SQL_Error']))
                    sys.exit('MariaDB Replication Health is NOT OK!')
                is_use_gtid = r_dict['Using_Gtid'].strip()
                if is_use_gtid != 'Current_Pos':
                    if is_use_gtid != 'Slave_Pos':
...
if __name__ == '__main__':
    ...
    # 第三步都检测完毕后，打印MariaDB复制成功信息。
    print('')
    logging.info('MariaDB Replication Health is OK.')
